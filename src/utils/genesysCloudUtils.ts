...
export function getAgentByUserId(id: string) {
  ...
  return usersApi
    ...
    .then((data: any) => {
      ...
      const imageUri: string = data.images?.find(
        (i: any) => i.resolution === "x96"
      )?.imageUri;
...
//   const body: any = {
//     interval: `${startInterval.toISOString(true)}/${endInterval.toISOString(
//       true
//     )}`,
//     order: "asc",
...
//         conversationData?.conversations?.filter((conversation: any) => {
//           return (
//             conversation.participants?.[0]?.sessions?.[0]?.mediaType === "voice"
//           );
//         }) || [];
...
/**
 * Get active conversation for the logged in user
 *
 * @param queueId the queue's id
 * @returns active conversation response
 */
export function getCurrentUserConversationsCalls() {
  return conversationApi
    .getConversationsCalls()
    .then((currentConvoData: any) => {
      console.log("CURRENT USER CONVERSATION DATA", currentConvoData);
      return currentConvoData;
    });
}
